...
function buildLinks() {
	return {
		getStarted: "v8/getting-started/first-stepper",
		migration: "v8/migration/v8",
		ownership: "v8/guides/local-and-shared-state",
		navigation: "v8/guides/navigation",
		reset: "v8/api/react/stepper-instance",
		instance: "v8/core-concepts/stepper-instance",
		primitives: "v8/guides/primitives",
		github: GITHUB,
	};
}
			...
			<div className="relative mx-auto grid w-full max-w-7xl gap-12 px-4 py-16 lg:grid-cols-[1.15fr_0.85fr] sm:px-6 lg:items-center lg:py-24">
				<div className="lp-rise" style={{ animationDelay: "0ms" }}>
					...
					<p className="mt-5 max-w-xl text-base text-muted-foreground sm:text-lg">
						Define your steps once. Keep state local or share it across
						components, validate before moving, and handle every navigation
						result with typed confidence. Your markup, your forms, your flow.
					</p>
...
function V8Highlights({ links }: { links: Links }) {
	const changes = [
		{
			number: "01",
			title: "Choose where state lives",
			body: "Each useStepper() call creates a local flow. A Provider shares a flow through useStepperContext(), with selectors for focused subscriptions.",
			link: links.ownership,
			action: "Try local and shared flows",
		},
		{
			number: "02",
			title: "Know how every move ends",
			body: "Navigation returns a typed result. Check accepted, then handle the reason when a guard, a boundary, or another rule prevents the move.",
			link: links.navigation,
			action: "Explore navigation results",
		},
		{
			number: "03",
			title: "Save and complete together",
			body: "Pass data and complete to one navigation action. They commit only when the move is accepted. Reset restores the flow’s initial state.",
			link: links.reset,
			action: "See the instance API",
		},
	];
	...
	return (
		<Section className="border-t bg-primary/[0.03]">
			<div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
				...
				>
					Migrating from v7 <ArrowRight className="size-4" aria-hidden />
				</DocLink>
			</div>
			<div className="mt-10 grid gap-8 lg:grid-cols-3">
				{changes.map((change) => (
					<div
						key={change.number}
						className="flex flex-col border-t border-primary/20 pt-5"
					>
						...
						>
							{change.action} <ArrowRight className="size-3.5" aria-hidden />
						</DocLink>
					</div>
				))}
			</div>
		...
		<Section className="border-y bg-muted/20">
			<div className="grid items-center gap-10 lg:grid-cols-2">
				<div>
					...
					<p className="mt-4 text-muted-foreground">
						One typed definition powers independent local flows and shared
						providers. Choose <Code>useStepper()</Code> for local state or{" "}
						<Code>useStepperContext()</Code> to read a provider. Step ids stay
						inferred through navigation, rendering, and primitives.
					</p>
					<ul className="mt-6 space-y-3 text-sm">
						...
						<FeatureLine>
							<Code>useStepperContext(selector)</Code> — subscribe to the state
							you need
						</FeatureLine>
					</ul>
					<DocLink
...
function Features() {
	const features = [
		...
		},
		{
			icon: Blocks,
			title: "React 18 & 19",
			body: "Server-render your flow, then hydrate it on the client. Use client components in React Server Component apps.",
		},
	];
	...
	return (
		<Section>
			<div className="lp-rise mx-auto max-w-2xl text-center">
				...
				<p className="mt-4 text-muted-foreground">
					Stepperize is headless, with optional unstyled primitives. Start from
					31 copy-paste blocks built with primitives and Tailwind, paste them
					into your project, and own every line. No theme to fight, nothing to
					eject.
				</p>
