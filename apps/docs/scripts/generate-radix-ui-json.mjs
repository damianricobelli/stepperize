...
const registryRoot = path.join(__dirname, "..", "registry", "radix-ui", "blocks");
const outDir = path.join(__dirname, "..", "public", "r", "radix-ui");
...
const meta = {
	...
	"stepper-with-form": { title: "Stepper with form", description: "A stepper with a form using zod for validation", registryDependencies: ["button", "input", "label", "form"], dependencies: ["zod", "@stepperize/react"] },
...
for (const name of blocks) {
	...
	const item = {
		...
		files: [
			{
				path: `registry/radix-ui/blocks/${name}/page.tsx`,
				content: pageContent,
				type: "registry:page",
				target: `app/${name}/page.tsx`,
			},
			{
				path: `registry/radix-ui/blocks/${name}/components/${name}.tsx`,
	...
	console.log(`Wrote r/radix-ui/${name}.json`);
}
