...
The stepper returned by `useStepper()` includes `onBeforeTransition(cb)` and `onAfterTransition(cb)`. Call them to register callbacks that run on every `next()`, `prev()`, or `goTo()`. The callback receives a `TransitionContext` with `from`, `to`, `metadata`, `statuses`, `direction`, `fromIndex`, `toIndex`. Return `false` from the before callback to cancel the transition.
...
const MyStepperComponent = () => {
  ...
  React.useEffect(() => {
    stepper.onBeforeTransition(async (ctx) => {
      ...
      return isValid; // false cancels transition
    });
    stepper.onAfterTransition((ctx) => {
      console.log(`Moved from ${ctx.from.id} to ${ctx.to.id}`);
    });
  }, [stepper]);
  ...
  return (
    <div>
      ...
      <button onClick={() => stepper.goTo("first")}>Go to first</button>
    </div>
  );
};
...
Methods on the object returned by `useStepper()`:

| Name | Type | Description |
| ---- | ---- | ----------- |
| `onBeforeTransition` | `(cb) => void` | Register callback that runs before any transition; callback can return `false` to cancel |
| `onAfterTransition` | `(cb) => void` | Register callback that runs after any transition |
...
Type guard to check if stepper initialization is complete. Useful when using async initial state.
