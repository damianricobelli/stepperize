...
const StepContent = () => {
  ...
  return (
    <div>
      {stepper.flow.when("first", (step) => <p>Starting with {step.title}</p>)}
      {stepper.flow.when("second", (step) => <p>In the middle: {step.title}</p>)}
    </div>
  );
};
...
const StepNavigation = () => {
  const stepper = useStepper();
  return (
    <button onClick={stepper.state.isLast ? () => stepper.navigation.reset() : () => stepper.navigation.next()}>
      {stepper.flow.when("last", () => "Reset", () => "Next")}
    </button>
  );
};
```
...
const StepContent = () => {
  const stepper = useStepper();
  return <div>Current: {stepper.state.current.data.title}</div>;
};
```
