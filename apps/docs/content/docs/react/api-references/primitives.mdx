...
| --------- | ----------- | ----------- |
| **Stepper.Root** | `role="group"`, `aria-label="Stepper"` | Groups the widget; override with your own `aria-label` if needed. |
...
| Key | Action |
| --- | ------ |
| **ArrowRight** / **ArrowDown** | Move focus and activation to the next step (wraps at end for arrows only when you implement it; current behavior: no wrap). |
| **ArrowLeft** / **ArrowUp** | Move focus and activation to the previous step. |
