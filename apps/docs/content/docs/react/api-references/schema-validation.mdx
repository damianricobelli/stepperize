...
function MyForm() {
  ...
  // Step by ID
  const personalStep = stepper.query.get("personal");
...
<Callout type="info">
  Steps are plain objects. If you add a `schema` property, access it as `stepper.state.current.data.schema` or `stepper.query.get(id)?.schema`. The last step can have an empty schema or no schema; check for existence when needed.
</Callout>
