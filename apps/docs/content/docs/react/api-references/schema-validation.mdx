...
Once you've defined your stepper with schemas, you can access them through the stepper instance:
...
function MyForm() {
  ...
  // Access schema from current step
  const currentSchema = stepper.current.data.schema;

  // Access schema from specific step
  const personalSchema = stepper.step("personal").data.schema;
  const addressSchema = stepper.step("address").data.schema;
...
<Callout type="info">
  ...
  because stepperize infers all the steps, and when you want to access your
  `stepper.current.data.schema`, one of them could be undefined. To avoid this, you can
