...
function MyForm() {
  ...
  // Step by ID
  const personalStep = stepper.get("personal");
...
<Callout type="info">
  Steps are plain objects. If you add a `schema` property, access it as `stepper.current.schema` or `stepper.get(id)?.schema`. The last step can have an empty schema or no schema; check for existence when needed.
</Callout>
