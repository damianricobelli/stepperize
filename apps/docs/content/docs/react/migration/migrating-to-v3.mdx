---
title: Migrating to v3/v4
description: Learn how to migrate from v2 to v3 and v4
...
<Callout type="info">
  In v6, state and flow are namespaced: use `stepper.state.current` (not `stepper.current`), `stepper.state.all` (not `stepper.all`), and `stepper.flow.switch(...)` (not `stepper.switch(...)`). See [Migrating to v6](./migrating-to-v6).
</Callout>
