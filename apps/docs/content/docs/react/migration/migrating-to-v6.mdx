...
## Lookup (formerly "query")

**Before:** If you used `stepper.query` or similar helpers.
...
The following are no longer part of the API:

- **Persistence / persist** — no built-in persist layer.
- **History** — no history stack or back/forward API.
- **Progress** — no dedicated progress-tracking API (derive from `state.current.index` and `state.all.length` if needed).
- **Initial statuses** — you can’t set initial statuses; status is derived from position (active / success / inactive) and is available via primitives (e.g. `useStepItemContext().status`) or when rendering the list yourself.
...
| Persist / history / progress | If present | Removed |
| UI components | Varies | `Stepper.Root`, `.List`, `.Item`, etc. |
