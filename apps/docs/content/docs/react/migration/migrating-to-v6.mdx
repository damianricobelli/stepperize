...
Stepperize v6 introduces a completely redesigned API with typed primitives, improved state management, and async navigation.

## Breaking Changes

### `defineStepper` signature: two parameters (steps array + config)

In v6, `defineStepper` takes **two arguments**: first the **array of steps**, second the **configuration object**. The single-object form is no longer supported.
...
- **First parameter:** the steps array (same step shape as before).
- **Second parameter:** optional config object (`initial`, `mode`, `onBeforeTransition`, `onAfterTransition`).
