...
All examples use the checkout flow from the [introduction](/docs/v7/forms):
...
[Preserve step state with React Activity](/docs/v7/guides/react-activity).
...
- **No form library** (or a separate form per step, or you want one
  ...
  [Standard Schema](/docs/v7/forms/schema-validation), so Zod, Valibot,
...
validate()](/docs/v7/forms/schema-validation) for the full validation flow
and the [navigation lifecycle](/docs/v7/core-concepts/navigation-lifecycle)
for the pipeline.
...
uncontrolled](/docs/v7/core-concepts/controlled-vs-uncontrolled).
...
checklist](/docs/v7/production/checklist#ssr-rsc-and-nextjs).
</Callout>
...
Next, see the [common problems](/docs/v7/forms/common-problems) these
patterns prevent — then pick an [implementation](/docs/v7/forms/react-hook-form).
