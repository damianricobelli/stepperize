...
The primitives expose their state through `data-*` attributes, which is how
you style them:

| Attribute | Values | Set on |
| --- | --- | --- |
| `data-complete` | Present (empty string) when explicitly completed; absent otherwise | `Item`, `Trigger`, `Indicator` |
| `data-state` | `active` · `inactive` | `Content` |
...
the status-vs-completion gotcha — see the [Styling guide](/docs/v8/guides/styling).
