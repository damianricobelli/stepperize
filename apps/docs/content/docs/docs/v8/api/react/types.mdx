...
| `NavigationResult` | Discriminated accepted/rejected navigation result. |
| `NavigationFailureReason` | Union of expected rejection reasons. |
| `GoToOptions` | Source data/completion and explicit `bypassPolicy`. |
| `ResetOptions` | `keepData` and `keepCompleted`. |
