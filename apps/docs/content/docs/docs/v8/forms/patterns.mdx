...
All examples use the checkout flow from the [introduction](/docs/v8/forms):
...
These snippets assume a shared `checkout.Provider` above the form and review components. Configure options on that Provider. For complete copyable files, see the React Hook Form, TanStack Form and Conform guides.
...
async function onStepSubmit(data: unknown) {
  await stepper.next({ data: data, complete: true });
}
```
...
[Preserve step state with React Activity](/docs/v8/guides/react-activity).
...
- **No form library** (or a separate form per step, or you want one
  ...
  [Standard Schema](/docs/v8/forms/schema-validation), so Zod, Valibot,
...
validate()](/docs/v8/forms/schema-validation) for the full validation flow
and the [navigation lifecycle](/docs/v8/core-concepts/navigation-lifecycle)
for the pipeline.
...
uncontrolled](/docs/v8/core-concepts/controlled-vs-uncontrolled).
...
checklist](/docs/v8/production/checklist#ssr-rsc-and-nextjs).
</Callout>
...
async function continueFromPersonal(data: PersonalValues) {
  ...
  if (data.orderType === "digital") {
    await stepper.goTo("payment", { bypassPolicy: true }); // deliberate branch
  }
}
```
...
Next, see the [common problems](/docs/v8/forms/common-problems) these
patterns prevent — then pick an [implementation](/docs/v8/forms/react-hook-form).
