...
pattern](/docs/v8/forms/patterns); most also appear in the general
[Troubleshooting](/docs/v8/getting-started/troubleshooting) page.
...
See [Pattern 3](/docs/v8/forms/patterns#pattern-3--validate-before-moving-forward).
...
**Fix:** pass the data **in** the navigation call, and only treat the transition as accepted when
...
[Pattern 1](/docs/v8/forms/patterns#pattern-1--save-a-steps-values).
...
See [Pattern 2](/docs/v8/forms/patterns#pattern-2--seed-a-step-from-its-draft).
If the problem is component-local state being destroyed by unmounts, use
[React Activity](/docs/v8/guides/react-activity) to keep inactive step
...
`goTo("review")`. See
[Pattern 5](/docs/v8/forms/patterns#pattern-5--edit-a-previous-step-then-return).
...
if (!moved.accepted) showErrorBanner();
```
...
```tsx
await stepper.next({ data: formValues, complete: true }); // completes the source
const allDone = stepper.steps.every((s) => stepper.isComplete(s.id));
```

See [Status vs
completion](/docs/v8/core-concepts/status-vs-completion).

---

Ready to build it for real? Pick an implementation:
[React Hook Form](/docs/v8/forms/react-hook-form),
[TanStack Form](/docs/v8/forms/tanstack-form), or
[Conform](/docs/v8/forms/conform).
