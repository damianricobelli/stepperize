...
`complete: true` during navigation or `setComplete(id)`, then style `[data-complete]` (Tailwind: `data-[complete]:bg-green-600`). The attribute is present with an empty value when complete and absent otherwise. See
[Status vs. completion](/docs/v8/core-concepts/status-vs-completion).

## Where to go next

- [Primitives](/docs/v8/api/react/primitives) — the components that emit these attributes.
