...
`Stepper.Root` creates and provides one stepper instance. The render prop and `useStepperContext()` read its shared instance; `useStepper()` creates independent local state.
...
Next: [read the primitives API reference](/docs/v8/api/react/primitives).

## v8 instance behavior

`List` and `Separator` inherit Root orientation unless overridden. `Content forceMount` retains inactive panels with `hidden` and `data-state="inactive"`. Trigger defaults to `type="button"`. `data-complete` reflects explicit completion. Each instance has independent context and DOM ids, including nested flows. `createStepperPrimitives` is internal; use the generated `definition.Stepper`.
