...
Choose the optional lightweight entry if you do not need generated `Stepper` components:

```tsx
import { defineStepper } from "@stepperize/react/headless";
```

It comes with the same package and retains hooks, Provider, selectors and validation. Its definition has no `Stepper` property. Use the main entry for the accessible UI primitives. See [bundle measurements and a complete example](/docs/v8/guides/performance#smaller-bundles-with-your-own-ui).
...
Stepperize supports React 18 and 19. It can run in JavaScript too, but you lose most of the type-safety that makes the library useful.

Next: [build your first stepper](/docs/v8/getting-started/first-stepper).
