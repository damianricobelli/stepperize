...
`checkout.useStepperContext()` throws when no matching owner exists. `checkout.useStepper()` always creates its own local instance. Use the context hook in shared descendants.
...
### Navigation returns a Promise containing a result
...
```tsx
// ❌ The Promise itself is always truthy.
if (!stepper.next()) return;

// ✅ await, then inspect the discriminant
...
Calling `useStepper()` in two sibling components always creates two independent instances, including under a shared `Provider`/`Root`. Moving one will not move the
other.
...
<checkout.Provider>
  <Panel />   {/* calls useStepperContext() */}
  <Footer />  {/* calls useStepperContext() */}
</checkout.Provider>
```

See [Shared state](/docs/v8/guides/shared-state).
...
that `stepper.goTo(id)` follows this policy too. For an intentional branch, pass `{ bypassPolicy: true }`; the guard still runs.
...
step has it. Narrow first with `stepper.match(...)` or `stepper.current.id`, or make
...
<Callout type="info">
Still stuck? Check the [FAQ](/docs/v8/getting-started/faq) or open an
