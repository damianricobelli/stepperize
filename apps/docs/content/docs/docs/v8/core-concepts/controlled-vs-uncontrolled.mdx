...
1. Choose **Next** with both checkboxes enabled. The request and the rendered step advance together.
2. Return the external step to `details`, disable **Apply onStepChange requests**, and choose **Next**. The request is accepted, but the rendered step stays put because the owner did not apply it.
3. Disable **Guard allows navigation** and choose **Next** to see a rejected request. Then change **External step**: the prop changes directly, bypassing the navigation lifecycle.

The last requested step records the most recent callback; external prop changes do not invoke that callback.

<InteractiveExample name="controlled" />
...
shared instance the same way. See [Sharing a stepper](/docs/v8/guides/shared-state).
