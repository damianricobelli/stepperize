...
export function PropsTable({
	...
	return (
		<Table>
			<TableHeader>
				<TableRow>
					<TableHead style={{ width: propHeaderFixedWidth ? "37%" : "auto" }}>Prop</TableHead>
				...
				{data.map(({ name, type, typeSimple, required, default: defaultValue, description }, i) => {
					return (
						...
						<TableRow key={`${name}-${i}`} style={{ whiteSpace: "nowrap" }}>
								...
								<div className="inline-flex items-center gap-2">
									...
									{Boolean(typeSimple) && Boolean(type) && (
										<Popover>
											...
											<PopoverContent side="top" align="center" className="max-w-[300px] w-full">
							...
							<TableCell>
								{defaultValue ? <Code>{defaultValue}</Code> : <Minus style={{ color: "var(--gray-8)" }} />}
							</TableCell>
						</TableRow>
					);
				})}
