...
export type Stepper<Steps extends Step[] = Step[]> = {
  ...
  get: <Id extends StepperGet.Id<Steps>>(
    id: Id
  ) => StepperGet.StepById<Steps, Id>;
   ...
   */
  match: <State extends StepperGet.Id<Steps>, R>(
    state: State,
    matches: StepperGet.Switch<Steps, R>
  ) => R | null;
};
...
export type StepperUtils<Steps extends Step[] = Step[]> = {
   ...
   */
  getNeighbors: <Id extends StepperGet.Id<Steps>>(
    id: Id
...
export namespace StepperGet {
  ...
  export type StepById<
    Steps extends Step[],
    Id extends StepperGet.Id<Steps>
