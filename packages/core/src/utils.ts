import type { Step, Stepper, StepperGet, StepperUtils } from "./types";
...
export function generateStepperUtils<const Steps extends Step[]>(...steps: Steps) {
	return {
		...
		get: (id) => {
			...
			return step as StepperGet.StepById<Steps, typeof id>;
		},
	...
	} satisfies StepperUtils<Steps>;
}

export function getInitialStepIndex<Steps extends Step[]>(steps: Steps, initialStep?: StepperGet.Id<Steps>) {
	return Math.max(
...
export function generateCommonStepperUseFns<const Steps extends Step[]>(
	...
	return {
		switch(when) {
			...
			return whenFn?.(currentStep as StepperGet.StepById<typeof steps, (typeof currentStep)["id"]>);
		},
