...
This is an example of a clickable stepper. Just add the `clickable` prop to the `Stepper` component to enable this feature.

With this enabled, the component will be keyboard accessible and you will be able to click on the steps to navigate.
...
<Callout type="info">
  If you notice, each trigger uses dataAttr and triggerProps. This is because the component is using the `a` tag to enable the clickable and navigate feature.
</Callout>